...
// overloading the PBS workload manager

// The interval (milliseconds) in which the queue data automatically reloads
// by calling qstat
// const AUTO_QSTAT_LIMIT = config["qstatRefreshRate"];

class PbsWidget extends Widget {
  // The table element containing PBS queue data.
  ...
  private user: string;
  // URL for calling qstat -u, used for user view
  private userViewURL: string;
  // URL for calling qstat, used for global view
  private globalViewURL: string;

  constructor() {
    super();
    this.id = "jupyterlab-pbs";
    this.title.label = "PBS Queue Manager";
    this.title.closable = true;
    this.addClass("jp-PbsWidget");
    ...
    // reference to this PbsWidget object for use in functions where THIS
    ...
    // The ajax request URL for calling qstat; changes depending on whether
    // we are in user view (default), or global view, as determined by the
    // toggleSwitch, defined below.
    this.userViewURL = URLExt.join(
      baseUrl,
      config["qstatURL"] + "?userOnly=true"
    );
    this.globalViewURL = URLExt.join(
      baseUrl,
      config["qstatURL"] + "?userOnly=false"
    );
    ...
    $(document).ready(function() {
        ...
        buttons: {
          buttons: [
            ...
            },
            {
              ...
              text: "Kill Job(s)",
              action: (e, dt, node, config) => {
                self.runOnSelectedRows("/qdel", "DELETE", dt);
              }
            },
            {
              extend: "selected",
              text: "Hold Job(s)",
              action: (e, dt, node, config) => {
                self.runOnSelectedRows("/qhold", "PATCH", dt);
              }
            },
            {
              extend: "selected",
              text: "Release Job(s)",
              action: (e, dt, node, config) => {
                self.runOnSelectedRows("/qrls", "PATCH", dt);
              }
            },
            {
              text: "Submit Job",
      ...
      $("#jupyterlab-pbs").append(toggleContainer);
      ...
      $("#jupyterlab-pbs").append(alertContainer);
      ...
      $("#jupyterlab-pbs").append(modalContainer);
      ...
      $("#submitScript").click(function(event) {
        event.preventDefault();
        self.submitJobScript(<string>$("#batchScript")
          .val()
          .toString());
        // Reset form fields
        document.forms["jobSubmitForm"].reset();
    ...
    $.when(userRequest).done(function() {
      $("#toggleSwitch").change(function() {
        if ((<HTMLInputElement>this).checked) {
          ...
          table.clear();
          let filteredData = self.dataCache.filter(function(value, index) {
...
/**
 * Activate the PBS widget extension.
 */
function activate(
  app: JupyterFrontEnd,
  ...
  // Declare a PBS widget variable
  let widget: PbsWidget;
  ...
  let tracker = new WidgetTracker<Widget>({ namespace: "pbs" });
